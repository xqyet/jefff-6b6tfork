...
public class BoatGlitch extends Module {
    ...
    public BoatGlitch() {
        super(Addon.CATEGORY, "BoatGlitch6b6t", "Dependency for BoatPhase - Don't touch");
    }
    ...
    private void onBoatMove(BoatMoveEvent event) {
        if (dismountTicks == 0 && !dontPhase) {
            if (boat != event.boat) {
                ...
                if (mc.player != null && mc.player.getVehicle() == event.boat) {
                    boat = event.boat;
                } else {
                    boat = null;
                }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (remountTicks > 0) {
            ...
            if (remountTicks == 0 && boat != null && mc.getNetworkHandler() != null) {
                mc.getNetworkHandler().sendPacket(
                    PlayerInteractEntityC2SPacket.interact(boat, false, Hand.MAIN_HAND)
                );
    ...
    private void onKey(KeyEvent event) {
        ...
        if (event.key == mc.options.sneakKey.getDefaultKey().getCode()
